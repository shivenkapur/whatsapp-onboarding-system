...
export default async function getMessages(
  ...
  let caregivers = await caregiverstoSendMessages(
    calendlyCaregivers,
    staffTrackerCaregivers,
    messageTrackerData
  );
...
async function caregiverstoSendMessages(
  calendlyCaregivers,
  staffTrackerCaregivers,
  messageTrackerData
) {
  ...
  for (let caregiverIndex in calendlyCaregivers) {
    ...
    let scheduledTime = new Date(
      calendlyCaregiver["Scheduled Date"] +
        " " +
    ...
    if (
      ...
      if (
        cellData.hasData(staffNumber) &&
        cellData.hasData(calendlyCaregiver["Potential CG"]) &&
        staffTrackerCaregiver["Status"] == "Not Onboarded"
      ) {
        if (
          cellData.hasNoData(
            messageTrackerData[calendlyCaregiver["Staff"]][
              "Sent Video Call Reminder Message to MQ"
            ]
          )
        ) {
          ...
          cellData.hasNoData(
            messageTrackerData[calendlyCaregiver["Staff"]][
              "Sent 1 hour Before Video Call Reminder Message"
            ]
          ) &&
...
async function getMessagesFromCaregivers(staffTrackerCaregivers) {
  ...
  for (let staffTrackerCaregiverIndex in staffTrackerCaregivers) {
    let staffTrackerCaregiver =
      staffTrackerCaregivers[staffTrackerCaregiverIndex];
    ...
    let messageSender = {
      ...
      messageType: staffTrackerCaregiver["Message Type"],
      Phone: staffTrackerCaregiver["Phone"],
    };
