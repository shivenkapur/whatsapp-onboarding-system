...
export default async function start(){
    ...
    for(let staffTrackerCaregiverIndex in staffTrackerData){
        ...
        if(staffTrackerCaregiver['Status'] == 'Onboarded'){
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'welcomeMessage'});
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'scheduleInterview'});
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'videoCallReminder'});

            let today = new Date();
            let dateOfOnboarding = new Date(staffTrackerCaregiver['Date']);

            if((today - dateOfOnboarding) >= 1000*3600*24*1)
                messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'nextSteps'})
        } else if(staffTrackerCaregiver['Status'] != 'Not Onboarded'){
            //Do Something in the future
        } else if(staffTrackerCaregiver['HKID'] == 1 && staffTrackerCaregiver['Cert'] == 1 && calendlyData[staffTrackerCaregiver['Old #']]){

            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'welcomeMessage'});
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'scheduleInterview'});

            let scheduledDate = new Date(calendlyData[staffTrackerCaregiver['Old #']]['Scheduled Date']);
            let today = new Date();

            if(scheduledDate.getTime() < today.getTime()){
                messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'videoCallReminder'});
            }
        } else if(staffTrackerCaregiver['HKID'] == 1 && staffTrackerCaregiver['Cert'] == 1){
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'welcomeMessage'});
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'scheduleInterview'});
        } else {
            messages.push({'Old #': staffTrackerCaregiver['Old #'], 'messageType': 'welcomeMessage'});
