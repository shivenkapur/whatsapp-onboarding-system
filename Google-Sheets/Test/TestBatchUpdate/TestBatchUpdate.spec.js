...
describe('Batch Data to Google Sheet', async function() {
    ...
    for(let sheetIndex in sheets){
        ...
        it('batch update Data to ' + sheet.SHEET_IDENTIFIER + ' ' + sheet.SHEET_NAME, async function() {
            
            if(Object.keys(TestData).includes(sheetIndex))
            {
                
                let sheetData = await api.batchUpdateGoogleSheet(sheetIndex, TestData[sheetIndex].TestCases)
                ...
                if(TestData[sheetIndex].Assert){
                    ...
                    for(let testCaseIndex in testCases){
                        ...
                        let confirmData = await api.getGoogleSheetData(sheetIndex, '!' + testCase.range.split('!')[1])
