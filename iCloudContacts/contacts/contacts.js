...
export default {
  importContacts: async function getContacts(page) {
    try {
      let settingsElement = await page.findByCssSelector(
        selectors.importContactsMenu
      );
      await settingsElement.click();

      let importVcard = await page.findByCssSelector(
        selectors.importContactsMenu
      );
  ...
  checkIfContactExists: async function checkIfContactExists(page, message) {
    try {
      let contactInputElement = await page.findByCssSelector(
        selectors.contactSearch
      );
      await contactInputElement.clear();
      await contactInputElement.sendKeys(message["Name"]);
      await new Promise((resolve) => setTimeout(resolve, 1000));
      let exists = await page.findByCssSelector(
        selectors.contactSelected,
        1000
      );
