...
export default async function updateMessageQueue(newMessages, messagePriority) {
    ...
    let messageQueueData = await googleSheets.getGoogleSheetData(INTERNAL_TOKEN, "", test = global.test);
    ...
    for (let newMessageIndex in newMessages)
    {
        ...
        if(!Object.keys(batchUpdate).includes(newMessage['Old #']) || 
            (messagePriority[newMessage['messageType']] > messagePriority[ batchUpdate[newMessage['Old #']]['messageType'] ] ) ){
            ...
            if(!found && (!Object.keys(append).includes(newMessage['Old #']) || 
            (messagePriority[newMessage['messageType']] > messagePriority[append[newMessage['Old #']]['messageType']])))
...
async function batchUpdateSheet(batchUpdate){
    ...
    for(let mesageIndex in batchUpdate){
        ...
        let sentNumber = message['Reminder Number'];
        
        let update = true;
        if(cellData.hasNoData(message['Reminder Number']))
            sentNumber = 1;
        else if(message['messageType'] != message['Message Type from MQ'])
            sentNumber = 1;
        else if(parseInt(message['Reminder Number']) < MAX_REMINDERS)
            sentNumber = (parseInt(message['Reminder Number']) + 1).toString()
        else
            update = false;
        
        if(update){
            sheetData.push({range: global.test + 'Message Queue!E' + message['Row'], values: [[ '' ]]})
            sheetData.push({range: global.test + 'Message Queue!C' + message['Row'], values: [[ message['messageType'] ]]})
            sheetData.push({range: global.test + 'Message Queue!F' + message['Row'], values: [[ sentNumber ]]})
        }
...
async function appendSheet(append){
    ...
    for(let mesageIndex in append){
        ...
        sheetData.push([message['Old #'], message['searchName'], message['messageType']
                        , message['Phone'], '', '1' ])

    }
    googleSheets.appendGoogleSheet(INTERNAL_TOKEN, sheetData, test = global.test);
}