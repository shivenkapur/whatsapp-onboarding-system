...
describe("Update Message Tracker Main Sheet", async function () {
  ...
  for (let testIndex in TestData) {
    ...
    it(
      "update message tracker Test Case: " + testIndex.toString(),
      async function () {
        let date = new Date();
        ...
        if (TestData[testIndex].Assert) assert.isOk(result);
        else assert.isNotOk(result);
      }
    );
  }
});
...
async function validate(testCase, date) {
  for (let testCaseIndex in testCase) {
    ...
    let confirmData = await googleSheets.getGoogleSheetData(
      INTERNAL_TOKEN,
      "!B" + row + ":F" + row
    );
