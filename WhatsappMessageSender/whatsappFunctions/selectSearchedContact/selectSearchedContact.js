...
export default async function selectSearchedContact(page) {
  ...
  let selectSearchedContact =
    selectors.selectSearchedContact.getSearchedContact(contactIndexes[0] + 1);
...
async function getContactIndexes(page) {
  ...
  let contactIndexes = await page.evaluate(
    (allContactsBox, contactMatchedText) => {
