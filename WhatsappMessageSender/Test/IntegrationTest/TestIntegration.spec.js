...
describe('Open Whatsapp', async function() {
    ...
    describe('Send Messages to Contacts', async function() {
        for(let messageIndex in TestMessages){
            ...
            it('Create New Message' + ' ' + messageIndex.toString(), async function() {
            ...
            it('Search Contact' + ' ' + messageIndex.toString(), async function() {
                let contactSearched = await searchForContact(page, message['message']['Name'])
            ...
            it('Send Message' + ' ' + messageIndex.toString(), async function() {
                messageSent = await sendMessage(page, message['message']['Text'], contactClicked)
                assert.equal(messageSent, assertValues['Send Message']);
            });

            it('Send Message Via WaMe' + ' ' + messageIndex.toString(), async function() {
                if(!messageSent)
                {    
                    let contactClicked = await getContactViaWaMe(page, message['message']['Phone'])
